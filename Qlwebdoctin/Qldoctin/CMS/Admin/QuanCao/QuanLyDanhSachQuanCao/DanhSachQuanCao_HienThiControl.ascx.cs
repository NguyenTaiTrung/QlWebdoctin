...
namespace Qlwebdoctin.Qldoctin.CMS.Admin.QuanCao.QuanLyDanhSachQuanCao
{
    public partial class DanhSachQuanCao_HienThiControl : System.Web.UI.UserControl
    {
        ...
        <tr id='madong_" + dt.Rows[i]["MaQC"] + @"'>
               ...
               <td class='cothinh'>
                 <img class='hinhdaidien'src='/pic/anhquangcao/" + dt.Rows[i]["AnhQuanCao"] + @"'/>
                 <img class='hinhdaidienhover'src='/pic/anhquangcao/" + dt.Rows[i]["AnhQuanCao"] + @"'/>
               </td>
               <td class='cotidtheloi'>" + dt.Rows[i]["ThuTucQC"] + @"</td>
               <td class='cotcongcu'>
                   <a href='Admin.aspx?modul=quangcao&modulphu=danhsachquangcao&thaotac=chinhsua&id=" + dt.Rows[i]["MaQC"] + @"' class='sua' title='Sửa'></a>
